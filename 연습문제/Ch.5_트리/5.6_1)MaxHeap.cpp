...
public:
	MaxHeap<T>(int theCapacity = 10) : capacity(theCapacity)
	{
	...
	void Push(const T& e)//삽입
	{
		...
		while (currentNode != 1 && heap[currentNode / 2] < e)//삽입하려는게 부모노드보다 더 큰 수라면 아래에서 위로 올라가는 구조, 위치 바꿔준다.
		{
			heap[currentNode] = heap[currentNode / 2];//부모노드로 이동
			currentNode /= 2;
		}
		heap[currentNode] = e;//자기보다 크지 않은 부모 있을 때까지 올라가서 넣어준다.
	}
	void Pop()
	{
		...
		{
			if (child < heapSize && heap[child] < heap[child + 1]) child++;//맨 위보다 큰 값이 있으면 점점 내려간다. 
